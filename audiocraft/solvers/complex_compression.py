...
class ComplexCompressionSolver(CompressionSolver):
    ...
    def preprocess(self, x, is_gen_or_eval=False):
        ...
        sr_unif = torch.ones(B) * self.sr_params.super_res_prob
        ...
        sr_unif = torch.ones(B) * self.denoise_params.denoise_prob
    ...
    def generate(self):
        ...
        if not self.cfg.logging.log_wandb:
            ...
            return
        assert self.sr_params.super_res_prob in [0, 1] and self.denoise_params.denoise_prob in [0, 1], "generate supports strict probability. set to 0 or 1"
