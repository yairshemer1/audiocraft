...
class ComplexSuperResDenoiseCompressionSolver(CompressionSolver):
    ...
    def run_model(self, x, **kwargs):
        x_noisy = x.clone()
        x_clean = kwargs['clean_data']

        if self.is_training:
            noises = x_noisy - x_clean
            shuffled_indices = torch.randperm(x_clean.shape[0])
            shuffled_noises = noises[shuffled_indices]
            x_noisy = x_clean + shuffled_noises

        x_clean_downsampled = julius.resample_frac(x_clean, self.sr_params.origin_sr, self.sr_params.target_sr)

        x, cond, y_noisy_downsampled = self.preprocess(x=x_noisy, is_gen_or_eval=kwargs.get('is_gen_or_eval', False))
